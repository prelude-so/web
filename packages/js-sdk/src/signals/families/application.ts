export default class Application {
  constructor(
    ...
    public platform: ApplicationPlatform,
    public implementedFeatures: ApplicationSdkFeatures,
  ) {}
  ...
  static async collect(): Promise<Application> {
    return new Application(
      "js-sdk",
      "1.0.0",
      await ApplicationPlatform.collect(),
      ApplicationSdkFeatures.NONE,
    );
  }
}
...
export class ApplicationPlatform {
  ...
  static async collect(): Promise<ApplicationPlatform> {
    const p = new ApplicationPlatform();
    const connection =
    ...
    p.indexedDbEnabled = await ApplicationPlatform.indexedDbEnabled();
    p.localStorageEnabled = ApplicationPlatform.localStorageEnabled();
    p.maybeHeadless = await ApplicationPlatform.maybeHeadless();
    p.mediaCapabilities = await ApplicationPlatform.getMediaCapabilities();
    p.multiTouchDevice = navigator.maxTouchPoints > 1;
    const plugins = ApplicationPlatform.getPlugins();
    if (plugins) {
      p.pluginsDigest = await plugins.sha256();
    }
    p.userAgent = navigator.userAgent;
    p.webGlEnabled = !!ApplicationPlatform.getWebGLContext();
...
export enum ApplicationSdkFeatures {
  NONE = 0,
  SILENT_VERIFICATION = 1 << 0,
}
